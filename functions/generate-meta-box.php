...
class theme_meta_box{
	...
	function theme_meta_box($info, $options){
		...
		add_action('admin_menu', array(&$this, 'theme_add_metabox'));
		add_action('save_post', array(&$this, 'theme_save_metabox'));
	
	}
	
	function theme_add_metabox(){
		...
		if ( function_exists('add_meta_box') and is_array($this->location) ) :
			foreach($this->location as $loc):
				...
				$this->box_name,
				$this->box_title,
				array(&$this, 'theme_display_metabox'),
				$loc
				);
	...
	function theme_display_metabox(){
